...
///////////////////////////// FileACL - assumes this file is in path

function io_acl_fileacl(sOpt,sFolder,sTrustee,bNotThread){
	try{
		bNotThread = bNotThread ? true : false
		if(sOpt == "change"){
			return (oWsh.Run("%comspec% /c fileacl.exe \"" + sFolder + "\" /S " + sTrustee + ":RWXD | find /i \"error\">nul",oReg.hide,bNotThread) != 0)
		}
		else if(sOpt == "full"){
			return (oWsh.Run("%comspec% /c fileacl.exe \"" + sFolder + "\" /S " + sTrustee + ":F | find /i \"error\">nul",oReg.hide,bNotThread) != 0)
		}
		return false
	}
	catch(e){
		pmt_common_error(2,e);
		return false;
	}
}
