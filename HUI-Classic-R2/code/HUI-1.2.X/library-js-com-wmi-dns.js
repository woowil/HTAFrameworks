	...
	try{
		...
		if(sOpt == "gettxttypes"){			
			...
			*/
			...
			var oClass = wmi_dns_getclass(oService,sClass)
			sOwner = js_str_isdefined(sOwner) ? sOwner : sServer + "." + sContainerName
	...
	try{
		...
		if(sOpt == "getserial"){
			...
			if(oClass = wmi_win32_class("simple",sClass,oService,sQuery,"\n")){
				for(var i = 0, len = oClass.length; i < len; i++){
					for(var o in oClass[i]){
						if(oClass[i][o].name == "SerialNumber"){
							return oClass[i][o].value
						}
					}
				}
			}
