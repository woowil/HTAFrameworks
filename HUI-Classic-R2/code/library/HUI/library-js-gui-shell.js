...
function js_shl_open(sFoldersFiles){
	try{
		if(typeof(oShX) != "object") return false
		for(var i = 0, len = arguments.length; i < len; i++){
		    if(oFso.FolderExists(arguments[i]) || oFso.FileExists(arguments[i])){		    	
				oShX.Open(arguments[i]);
		    }
		}
    }
	catch(e){
...
function js_shl_setwindow(sOpt){
	try{
	   if(typeof(oShX) != "object") return false
	    if(sOpt == "minimizeall") oShX.MinimizeAll();
	    else if(sOpt == "restore") oShX.UndoMinimizeAll();
	    else if(sOpt == "minimize") oShX.Minimize();
		else if(sOpt == "refresh"){
			oShX.MinimizeAll();
			oShX.UndoMinimizeAll();
		}
    }
