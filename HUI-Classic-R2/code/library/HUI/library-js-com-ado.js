...
/*
getCSVSQL("Z:\\Export-Bodo.csv")

function getCSVSQL(sFileCSV){
	...
	oConnection.ConnectionString = "DRIVER={Microsoft Text Driver (*.txt; *.csv)};" +
									"DefaultDir=c:\\temp;Dbq=" + oFso.GetParentFolderName(sFileCSV) + ";Extensions=asc,csv,tab,txt;Persist Security Info=False"
	//Print(oConnection.ConnectionString)
	oConnection.Open()
	
	var sQuery = "select distinct * from \"" + oFso.GetFileName(sFileCSV) + "\" where SiteName like '%Bergen%'"
	//Print(sQuery)
	//oRecordset = oConnection.Execute(sQuery)
	oRecordSet.open(sQuery,oConnection,3,3)
	
	//alert(">>"+this.oRecordSet.RecordCount)
	// Do not use this.oRecordSet.MoveFirst(), generates error 3021
	for(; !oRecordSet.EOF; oRecordSet.MoveNext()){
		Print(oRecordSet.Fields("Printer").value)
	}
	oRecordSet.close()
	delete oRecordSet
