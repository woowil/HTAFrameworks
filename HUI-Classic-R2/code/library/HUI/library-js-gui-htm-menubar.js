...
function getMainMenuBarMenuTopPos(menuBarObj, menuBarItemObj, menuObj, y) { // Private method
  if (y + menuObj.offsetHeight <= getClientHeight()) {
    ...
    if ((ie55 || ie6) && menuBarObj.mode == "static" && pageMode == 1) {
       var x = menuBarItemObj.offsetTop - menuObj.offsetHeight - getPropIntVal(menuBarObj, pt) + getPropIntVal(menuBarItemObj, pt) + getPropIntVal(menuBarItemObj, btw) - getScrollTop();
		return x
    }
    else {
...
*/
function popUpMenuBarMenu(menuBarObj, menuBarItemObj, menuObj,menuObjLayer) { // Private method
  var x;
  var y;
  ...
  if (ie && menuObj.mode == "fixed") {
    ...
    menuObj.initialTop = parseInt(menuObj.style.top) - getScrollTop();
 	...
 	menuObj.style.visibility = "visible";
  	//alert(menuObj.clientWidth + " " + menuObj.clientHeight + " "+menuObj.id)
  	//alert(l.clientWidth + " " + l.clientHeight + " " + l.id)
}
  ...
  for (var i = 0, len = menuBarObj.childNodes.length; i < len; i++) {
    if(menuBarObj.childNodes[i].enabled && menuBarObj.childNodes[i].clicked) {
      ...
      if (menuBarObj.childNodes[i].menu) {
        hideMenus(menuBarObj.childNodes[i].menu.menuObj,menuBarObj.childNodes[i].menu.menuObjLayer);
      }
      break;
    }
  }
...
function menuBarItemOver(e) { // Private method
  ...
  else { // Clicked and mouse over
    var menuBarObj = this.parent.menuBarObj;
    //js_log_print("log_result","m 2")
	var ID = window.setTimeout("hideContextMenus()",0,"Javascript")
	for (var i = 0, len = menuBarObj.childNodes.length; i < len; i++) {
    ...
    if (this.enabled && (this.menu || this.actionOnClick)) {
      switch (menuBarObj.activateMode) {
        case "click":
          ...
          break;
        case "over":
          if (this.menu) {alert("over")
            this.onclick(e);
          }
          else {alert("not over clock")
            this.className = this.itemClassNameOver;
          }
          break;
      }
  ...
  if (this.enabled) {
    if (this.menu) {
      if (this.clicked) {
        ...
        //hideMenus(this.menu.menuObj,this.menu.menuObjLayer);
        this.clicked = false;
        this.parent.menuBarObj.activated = false;
      }
      else { // when i click on the menubar
      	var oDummy = setTimeout("hideContextMenus(true)",0,"JavaScript"); // Woody
		if(ClickedBarMenu) hideMenus(ClickedBarMenu.menuObj,ClickedBarMenu.menuObjLayer,ClickedBarMenu.menuObj.id); // Woody
        this.className = this.itemClassNameClick;
        if (this.iconObj && this.iconClassNameClick) {
          this.iconObj.className = this.iconClassNameClick;
        }
        popUpMenuBarMenu(this.parent.menuBarObj, this, this.menu.menuObj,this.menu.menuObjLayer);
        this.clicked = true;
        this.parent.menuBarObj.activated = true;
		ClickedBarMenu = this.menu
      }
    }
    else {
      if (this.actionOnClick) {
...
function jsDOMenuBar() { // Public method
  ...
  if (len > 1 && arguments[1].length > 0 && arguments[0] == "static") {
    ...
    //menuBarElm.onmouseout = hideMenuBarMenus;
  }
  ...
  // Woody
  menuBarElm.onselectstart = new Function("return false") // prevents selection
...
if (typeof(menuBarClassNameLayer) == "undefined") {
  var menuBarClassNameLayer = "jsdomenubardivLayer"; // Public field
}
