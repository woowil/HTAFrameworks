...
/*
	...
	http://forums.tohzhiqiang.per.sg/
...
function popUpSubMenu(menuItemObj){ // Private method
	...
	var menuObjLayer = menuItemObj.subMenu.menuObjLayer;	
	var x;
	var y;
	...
	showLayer(menuObjLayer,menuObj)
...
function popUpMainMenu(menuObj,e){ // Private method
	...
	showLayer(popUpMenuObj.menuObjLayer,menuObj)
...
function ContextMainMenu(menuObj,e){ // Woody right click in text box
	menuObj.style.left = (getMainMenuLeftPos(menuObj, getX(e)) + getScrollLeft()) + px;
	menuObj.style.top = (getMainMenuTopPos(menuObj, getY(e)) + getScrollTop()) + px;
	var display = popUpMenuObj.menuObj.style.display;
	showLayer(ContextMenuObj.menuObjLayer,menuObj)
...
function menuItemOver(e){ // Private method
	...
	if(previousItem){
		//js_log_print("log_result","yes")
		//previousItem.subMenu.menuObjLayer.style.visibility = previousItem.subMenu.menuObj.style.visibility = "hidden"
		...
		for(var i = 0, len = menuObj.childNodes.length; i < len; i++){
			if(menuObj.childNodes[i].enabled && menuObj.childNodes[i].subMenu){
				hideMenus(menuObj.childNodes[i].subMenu.menuObj,menuObj.childNodes[i].subMenu.menuObjLayer);
			}
		}
	}
	if(this.enabled){
		...
		if(this.subMenu){
			...
			//showLayer(this.parent.menuObjLayer,this.parent.menuObj)
			popUpSubMenu(this);
		}		
	...
	// Woody
	try{
		//this.style.backgroundColor = pmt.hta.color1;
		//this.style.borderColor = pmt.hta.color2;
	}
	catch(ee){
...
function menuItemClick(e){ // Private method
	...
	var t0 = hideContextMenus()
	if(this.parent.menuObj.mode == "cursor"){
		var t1 = setTimeout("hideVisibleMenus()",0,"JavaScript") // Woody
		var t2 = setTimeout("hideCursorMenus()",1,"JavaScript") // Woody
	}
	else if(this.parent.menuObj.mode == "absolute" || this.parent.menuObj.mode == "fixed"){
		var t1 = setTimeout("hideVisibleMenus()",0,"JavaScript") // Woody
		if(typeof(hideMenuBarMenus) == "function"){
			var t2 = setTimeout("hideMenuBarMenus()",1,"JavaScript") // Woody
		}
	}
}
...
function activatePopUpMenu(e){ // Private method
	...
	if(state == "visible"){		
		for(var i = 1; i <= menuCount; i++){
			...
			var menuObjLayer = getElmId("DOMenuLayer" + i);
			if(menuObj.mode == "cursor"){
				hideLayer(menuObjLayer)
		...
		//if(confirm("doit"))htm_dbg_object(targetElm)
		...
		if(canShowMenu(targetElm.tagName, targetElm.id, popUpMenuObj.menuObj.allExceptFilter, popUpMenuObj.menuObj.noneExceptFilter)){
			var oDummy = setTimeout("hideMenuBarMenus()",1,"JavaScript")
			popUpMainMenu(popUpMenuObj.menuObj,e);
		}
	}
}

function activateContextMenu(e){ // Woody
	e = e ? e : window.event;
	if(!ContextMenuObj){
		return;
	}
	...
	if(state == "visible"){
		...
		if(canShowMenu(targetElm.tagName, targetElm.id, ContextMenuObj.menuObj.allExceptFilter, ContextMenuObj.menuObj.noneExceptFilter)){
			ContextMainMenu(ContextMenuObj.menuObj,e);
		}
	}
}
...
*/
function ContextMenuHandler(e){ // Private method
	e = e ? e : window.event	
	var oTag = e.srcElement, t
	if(getX(e) > getClientWidth() || getY(e) > getClientHeight()){
		return;
	}
	else if(typeof(e) == "object" && e.button == 1){ // Left Click
		if(aMenus.length == 0) return;
		hideContextMenus()
		var t1 = setTimeout("hideVisibleMenus()",1,"JavaScript") // Woody
		var t2 = setTimeout("hideMenuBarMenus()",2,"JavaScript") // Woody
		/*
		if(popUpMenuObj){
			var state = popUpMenuObj.menuObj.style.visibility;
	...
	else if(typeof(e) == "object" && e.button == 2){ // Right Click		
		hideContextMenus()
		var t1 = setTimeout("hideVisibleMenus()",0,"JavaScript") // Woody
		if(typeof(hideMenuBarMenus) == "function"){
			var t2 = setTimeout("hideMenuBarMenus()",0,"JavaScript") // Woody
		}
		/*
		if((t=htm_doc_getselect()) != ""){				
			clipboardData.setData("Text",(ContextText=t)) // Must do this!!
			ContextRange = document.selection.createRange()
		}
		*/
		if(ContextMenuObj && (oTag.tagName == "TEXTAREA" || (oTag.tagName == "INPUT" && oTag.type == "text") )){ // Woody
			var state = ContextMenuObj.menuObj.style.visibility;
			if(state == "visible" && (hideValue == 0 || hideValue == 2)){
				activateContextMenu(e);
			}
...
*/
function addMenuItem(menuItemObj,sCtrlKey){ // Public method
	if(menuItemObj.displayText == "-"){
		var hrElm = createElm("hr");
		hrElm.noshade = true
		var itemElm = createElm("div");
		itemElm.appendChild(hrElm);		
		itemElm.innerHTML = '<table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td><hr size="1" noshade width="100%"></td></tr></table>';
		...
		itemElm.innerHTML = '<table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td>' + itemElm.innerHTML + '</td><td align="right">' + (sCtrlKey ? sCtrlKey : "&nbsp;") + '</td></tr></table>';
		this.menuObj.appendChild(itemElm);
		itemElm.parent = this;
...
function setItem(sOpt,oMenuItem,sOpt3){
	try{
		if(sOpt == "setactive"){
...
function jsDOMenu(){ // Public method
	...
	// Woody
	menuElm.onselectstart = new Function("return false") // prevents selection
	if(len > 1 && arguments[1].length > 0){
		switch (arguments[1]){
			case "cursor":
	...
	this.menuObjLayer = createLayer(this.menuObj) 
...
function hideLayer(menuObjLayer){ // Public method
...
function showLayer(menuObjLayer,menuObj){
	try{
		menuObjLayer = menuObjLayer ? menuObjLayer : createLayer(menuObj)
		var id = (menuObjLayer.id).replace(/DOMenuLayer([0-9]+)/ig,"$1")
...
function createLayer(menuObj){
	var menuObjLayer = createElm("div");
	var id = (menuObj.id).replace(/DOMenu([0-9]+)/ig,"$1")
	menuObjLayer.innerHTML = '<iframe id="oFrame' + id + '" _ALLOWTRANSPARENCY=true APPLICATION=no style="height:100%;width:100%;" frameborder="0" border="0" src=null></iframe>'; // This is a fix to 
	menuObjLayer.id = "DOMenuLayer" + id;
 	menuObjLayer.className = menuClassNameLayer
	hideLayer(menuObjLayer)
...
function hideAllMenus(){ // Public method
	try{
		var oDummy = setTimeout("hideContextMenus()",0,"JavaScript")
		for(var i = 1; i <= menuCount; i++){
			var menuObj = getElmId("DOMenu" + i);
			var menuObjLayer = getElmId("DOMenuLayer" + i);
...
function hideContextMenus(bIgnore){ // Woody
	try{
		hideLayer(popUpMenuObj.menuObjLayer)
		popUpMenuObj.menuObj.style.visibility = "hidden";
		hideLayer(ContextMenuObj.menuObjLayer)
		ContextMenuObj.menuObj.style.visibility = "hidden"
		if(bIgnore) return;
		for(var i = 0, len = aMenus.length; i < len; i++){
...
menuObj						: Required. Menu object that specifies the menu and all its submenus to 
										 be hidden.
*/
function hideMenus(menuObj,menuObjLayer){ // Public method
	refreshMenuItems(menuObj);
	for(var i = 0, len = menuObj.childNodes.length; i < len; i++){
		if(menuObj.childNodes[i].enabled && menuObj.childNodes[i].subMenu){
			hideMenus(menuObj.childNodes[i].subMenu.menuObj,menuObj.childNodes[i].subMenu.menuObjLayer);			
...
function setContextMenu(menuObj){ // Woody
...
if(typeof(menuClassNameLayer) == "undefined"){
	var menuClassNameLayer = "jsdomenudivLayer"; // Public field
}
...
function htm_menu_error(iOpt,oErr){
	try{
