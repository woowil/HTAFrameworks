...
function wmi_wbem_service(sComputer,sNameSpace,sUser,sPass,sImpersonation,sLocale,sAuthentication,sAuthority,iPrivilege,iSecurityFlags){
	try{ // http://www.microsoft.com/technet/scriptcenter/scrguide/sas_wmi_vzbp.asp
		...
		var oService = false;
		if(sComputer == "." || js_sys_localhost(sComputer)){// Local Connection
			sImpersonation = sImpersonation ? sImpersonation : "Impersonate";
			sAuthentication = sAuthentication ? sAuthentication : "Connect"; // Connect: Authenticates the credentials of the client only when the client establishes a relationship with the server.
...
function wmi_class_namespaces(bAll,oService,sComputer,sRoot,sUser,sPass,sImpersonate,sLocale,sAuthentication,sAuthority,iPrivilege,aNameSpaces){
	try{
		...
		for(var oItem = "", oEnum = new Enumerator(oNameSpace); !oEnum.atEnd(); oEnum.moveNext()){
			oItem = oEnum.item();
	...
	try{
		...
		if(sLanguage.toLowerCase() == "jscript"){ // JScript			
			...
			if(bLocal){
				...
				sHtml = sHtml + "\n\t\tif(iPrivilege == -1){"
				sHtml = sHtml + "\n\t\t\tfor(var p = 1; p < 27; p++){ // Adds all available privileges"
				sHtml = sHtml + "\n\t\t\t\ttry{"
				sHtml = sHtml + "\n\t\t\t\t\toService.Security_.Privileges.Add(p); // http://msdn.microsoft.com/library/default.asp?url=/library/en-us/wmisdk/wmi/wbemprivilegeenum.asp"
				sHtml = sHtml + "\n\t\t\t\t}"
				sHtml = sHtml + "\n\t\t\t\tcatch(ee){}// Invalid Parameter"
				sHtml = sHtml + "\n\t\t\t}\n\t\t}"
			...
			for(var oItem, oEnum = new Enumerator(oClass.properties_); !oEnum.atEnd(); oEnum.moveNext()){
				...
				if(oItem.IsArray){ // Only VBArrays
					...
					*/					
					...
					/*
					sHtml = sHtml + "\n\t\t\tif(oItem." + oItem.Name + " != null){ // DateTime object";
...
function wmi_class_parametertype(oProperty){
	try{
		...
		switch(oProperty.CIMType){
			...
			case 8 : {
				oType.type1 = "String", oType.type2 = "";
				break;
			}
			case 11 : {
	...
	try{
		...
		if(typeof(oInpars) != null){ // Some method parameters are null
			try{ // Properties may not exist
				aParametar = new Array();
				var oParams = oInpars.Properties_;
				for(var oItem = "", oType, i = 0, oEnum = new Enumerator(oParams); !oEnum.atEnd(); i++, oEnum.moveNext()){
					oItem = oEnum.item(); // EX: SetPowerState
					aParametar[i] = new Object(); // EX: PowerState 
					aParametar[i].name = aParametar[i].type1 = aParametar[i].typestr = aParametar[i].value = "";
					oType = wmi_class_parametertype(oItem)
					aParametar[i].name = oItem.Name // EX: PowerState  
					aParametar[i].type1 = oType.type1; // EX: Integer
					aParametar[i].typestr = oType.text; //EX: Integer (USINT16)
					aParametar[i].cimtype = oItem.CIMtype;
					// Get the description
					aParametar[i].desc = (tmp = wmi_obj_description(oItem)) ? tmp : "";
				}
			}
			catch(ee){ }
			aParametar.name = oMethod.Name;
			aParametar.desc = wmi_obj_description(oMethod); // Get description
		}
	}
	catch(e){
		wmi_log_error(2,e);
...
function wmi_meth_outparameter(oMethod){
	try{
		...
		if(typeof(oOutpars) != null){ // Some method parameters are null
			try{ // Properties may not exist
				oParametar = new Object();
				for(var oItem = "", i = 0, oEnum = new Enumerator(oOutpars.Properties_); !oEnum.atEnd(); i++, oEnum.moveNext()){
					oItem = oEnum.item(); // EX: SetPowerState
					oParametar.name = (oItem.Name)
					oParametar.type1 = (wmi_class_parametertype(oItem)).type1;
					oParametar.typestr = (wmi_class_parametertype(oItem)).text;
					oParametar.value = oItem.Value;
					oParametar.cimtype = oItem.CIMtype;		
				}
			}
			catch(ee){}			
		}
...
function wmi_win32_class(sOpt,sClass,oService,sQuery,sBreak,sComputer,sNameSpace,sUser,sPass,sImpersonation,sAuthentication,oTBody){
	try{
		...
		var aProperties, sGetObject, bAsk = true		
		var oRe = new RegExp("Select[ \t]+(.+)[ \t]+from[ \t]+(.+)","ig")		
		var bSelect = false, oRe1
		if(!oService) return false
		if(sQuery.match(oRe)){
			var oSelect = new ActiveXObject("Scripting.Dictionary");
		...
		if(aProperties){			
			...
			for(var oItem, i = 0, oEnum = new Enumerator(oColItems); !oEnum.atEnd() && !js.pro.stopit; oEnum.moveNext(), i++){
				...
				if(!bTBody){
					...
					oCell = oRow.insertCell(), oCell.innerText = "instance - " + js_str_number(i)
					oCell.className = "cHead2", oCell.colSpan = 5
				}
				for(var j = 0, t, n, v; j < len && !js.pro.stopit; j++, r++){
					n = aProperties[j].name
				...
				if(bTBody && bAsk){
					...
					if(oWsh.Popup("The WQL query: " + sQuery + " is taking awfully a lot of time. Continue?",15,"WMI popup",32 + 1) == 2) break;
					bAsk = false
				}
...
function wmi_reg_param(oService,hDefKey,sKeyPath,sKeyName,sMethod,sComputer,sUser,sPass){
	try{
		// http://www.serverwatch.com/tutorials/article.php/1476831
		// http://www.microsoft.com/technet/treeview/default.asp?url=/technet/scriptcenter/registry/ScrReg15.asp
		// http://msdn.microsoft.com/library/default.asp?url=/library/en-us/wmisdk/wmi/writing_wmi_scripts_in_jscript.asp
		oService = oService ? oService : wmi_wbem_service(sComputer,"root\\default",sUser,sPass);
		var oServiceReg = oService.Get("StdRegProv");
		var oMethod = oServiceReg.Methods_.Item(sMethod); // sMethod: EnumKey, GetStringValue, GetDWORDValue
		var oInParam = oMethod.InParameters.SpawnInstance_();
		oInParam.hDefKey = hDefKey; // wmi.HKLM;
		oInParam.sSubKeyName = sKeyPath;
		if(sKeyName) oInParam.sValueName = sKeyName;
		var oOutParam = oServiceReg.ExecMethod_(oMethod.Name,oInParam);
		return oOutParam;
	}
	catch(e){
		wmi_log_error(2,e);
...
function wmi_reg_enumvalues(oService,hDefKey,sKeyPath,sComputer,sUser,sPass){
	try{
		var oValues = false;
		if(oOutParam = wmi_reg_param(oService,hDefKey,sKeyPath,false,"EnumValues",sComputer,sUser,sPass)){
			oValues = new Object();
			try{
				oValues.names = oOutParam.sNames.toArray();
				oValues.types = oOutParam.Types.toArray();
			}
			catch(ee){ // Empty keys
				oValues.names = new Array();
				oValues.types = new Array();
			}
		}
		return oValues;
	}
...
function wmi_reg_enumkeys(oService,hDefKey,sKeyPath,sComputer,sUser,sPass){
	try{
		var oOutParam = wmi_reg_param(oService,hDefKey,sKeyPath,false,"EnumKey",sComputer,sUser,sPass);
		var oKeys = oOutParam.sNames.toArray();
		return oKeys;
	}
	catch(e){
		wmi_log_error(2,e);
...
function wmi_reg_getvalue(oService,hDefKey,sKeyPath,sKeyName,iRegType,sComputer,sUser,sPass){
	try{
		if(iRegType == wmi.REG_SZ) sMethod = "GetStringValue";
		else if(iRegType == wmi.REG_DWORD) sMethod = "GetDWORDValue";
		else if(iRegType == wmi.REG_MULTI_SZ) sMethod = "GetMultiStringValue";
		else return false;
		
		var oOutParam = wmi_reg_param(oService,hDefKey,sKeyPath,sKeyName,sMethod,sComputer,sUser,sPass);
		if(iRegType == wmi.REG_MULTI_SZ) return oOutParam.sValue.toArray();
		else return oOutParam.sValue;
	}
	catch(e){
		wmi_log_error(2,e);
		return false;
