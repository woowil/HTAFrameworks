...
function htm_nav_res_search(win,iOpt){
	try{
		htm.br.exec = true;
		if(str = win.location.search){
			switch(iOpt){
				...
				default : {
					htm.br.exec = false;
					htm_log_error(3,"Error calling function htm_nav_res_search() with argumentiOpt.");
					break;
				}
			}
		}
...
function htm_div_html(sOpt,sOpt2,sOpt3,sOpt4,bSort){
	try{
		if(sOpt == "settable"){
			...
			for(var i = 0, c, len = r.length; i < len; i++){
				c = r[i].split(sOpt4)
				t = t + "\n<tr>"
...
function htm_div_table(sOpt,sTable,iIndex1,iIndex2,bRefresh,bNowrap){
	try{ // http://msdn.microsoft.com/library/default.asp?url=/workshop/author/dhtml/reference/methods/insertadjacenthtml.asp
		...
		if(!htm_str_isdefined(oTable)) return false // Must be done otherwise.. your system will crash if the object is not found
			...
			iIndex1 = js_str_isnumber(iIndex1) ? iIndex1 : 1;
		...
		for(var i = 0, len = aArray.length; i < (len-1); i++){
		    for(var j = i+1; j < len; j++){
		    	if(opt == 1) bSort = (aArray[j] < aArray[i]);
					...
					var a = bIgnoreCase ? (aArray[j][sObject]).toUpperCase() : aArray[j][sObject];
					var b = bIgnoreCase ? (aArray[i][sObject]).toUpperCase() : aArray[i][sObject];
					bSort = (a < b);
				}
				if(bSort){
					var dummy = aArray[i];
...
function htm_ary_duplicate(opt,aArray1,aArray2,bIgnoreCase){
	try{
		...
		if(opt == 1){ // Return distinct Array list of arrays
			if(aArray2 && typeof(aArray2) == "object" && aArray2.length > 0) aArray1 = aArray1.concat(aArray2)
			aArray1.sort();
...
function htm_frm_select(sOpt,oForm,oSel,iIndex,sValue,sText,sOpt7,sOpt8,sOpt9,sOpt10){
	try{
		//if(typeof(sOpt) != "string" || typeof(sOpt) != "number") return false
		if(typeof(oSel) == "string" && typeof(oForm) == "object") oSel = oForm[oSel];
		...
		if(typeof(oSel) != "object" || oSel.tagName != "SELECT") return false
			...
			while(i-- > iIndex){
				oSel.options[iIndex].removeNode()// = null; // Also possible to use options.removeNode()
			}
...
function htm_str_kill(){
	try{ // delete: http://users.adelphia.net/~daansweris/js/special_operators.html
		for(var i = arguments.length-1; i > 0; i--){
			if(typeof(arguments[i]) == "object") {
				try{
					if(arguments[i].length){ // Array
						for(var j = arguments[i].length-1; j >= 0; j--) delete (arguments[i])[j]
						arguments[i].length = 0
						arguments[i] = null
					}
					else {
						for(o in arguments[i]) htm_str_kill(o)
					}
				}
				catch(ee){}
			}
