...
__H.include("HUI@Util@List.js")

__H.register(__H.Util.List,"LinkedDouble","Linked List Double",function LinkedDouble(oData,sName){
	...
	this.size = 0;
	//this.size++;
	var MyList = __H.Util.List.LinkedDouble
	// TEST functions
	/////////////////////////////////////////////////////////////
	
	this.testNode = function(){
		var oFruits = new MyList("apple"), oFruits1, oFruits2
	...
	this.reverse = function(oNode){
		...
		var oReverse = null
		while(oNode.hasPrev()){
			oReverse = oReverse ? oReverse : new MyList(oNode.data,oNode.name) 
			oNode = oNode.prev
	...
	this.addNext = function(oNode,oData,sName){
		oNode = this.isNode(oNode) ? oNode : new __H.Util.List.LinkedDouble(oData,sName)
		oNode.prev = this
	...
	this.shuffle = function(oNode){ // NOT working 100%
		oNode = this.isNode(oNode) ? oNode : this
		oNode = oNode.getHead()
		var oShuffle = new __H.Util.List.LinkedDouble()
	...
	this.toNode = function(aNode){
		...
		if(typeof(aNode) == "object"){
			for(var i in aNode){
				if(aNode.hasOwnProperty(i)){
					if(!oNode) oNode = new MyList(aNode[i])
...
var __HLinked = new __H.Util.List.LinkedDouble()