...
function xml_dtd_entities(sOpt,oXml,sQualName,sPublicId,sSystemId,sSubset,sType){
	try{
		if(!js_str_isdefined(oXml)) return false
			...
			for(var oItem, oEnum = new Enumerator(oEntities); !oEnum.atEnd(); oEnum.moveNext()){
				oItem = oEnum.item();
				o = new Object()
...
function xml_dtd_notation(sOpt,oXml){
	try{ // http://www.devguru.com/technologies/xmldom/quickref/obj_notation.html
		if(!js_str_isdefined(oXml)) return false
			...
			for(var oItem, oEnum = new Enumerator(oNotations); !oEnum.atEnd(); oEnum.moveNext()){
				oItem = oEnum.item();
				o = new Object()
...
function xml_log_errorxml(oErr){
	try{		
		if(oErr.errorCode != 0){
			...
			sError += "\nXML CODE :\t" + (js.err.errorCode = oErr.errorCode);
