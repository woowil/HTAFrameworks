...
function io_ini_isfile(sFile,sExt){
	try{
		sExt = sExt ? "|" + sExt : "";
		var oRe = new RegExp("ini|inf"+ sExt,"ig")
		if(oFso.FileExists(sFile) && (sExt = oFso.GetExtensionName(sFile))){
			return sExt.match(oRe) ? true : false;
		}
		return false
	}
	catch(e){
...
function io_ini_iskey(sLine,iLine){
	try{
		...
		var sLineOrg = sLine, sLine = js_str_trim(sLine), oRe = new RegExp("[ \t]+","g");
...
/**
	...
	Exception:  On Error
*/
function io_ini_getkeys(sFile,sParameter,sExt){
	try{
		var sResult = false
		if(js_str_blankline(sParameter)) return false;
		else if(io_ini_isfile(sFile,sExt) && js_str_isdefined(sParameter)){
			var oFile = oFso.OpenTextFile(sFile,oReg.read,false,oReg.TristateUseDefault);
			while(!oFile.AtEndOfStream){
...
/**
	...
	Exception:  On Error
*/
function io_ini_addkey(sFile,sParameter,sKeyName,sKeyValue,sExt){
	try{
		if((iLine = io_ini_parameterexists(sFile,sParameter,sExt)) && sKeyName && sKeyValue){
			var oFile = oFso.OpenTextFile(sFile,oReg.read,false,oReg.TristateUseDefault);
			var oTmpFile = oFso.OpenTextFile(sFile + ".tmp",oReg.write,true,oReg.TristateUseDefault);
			while(!oFile.AtEndOfStream){
				sLine = oFile.ReadLine();
			...
			while(!oFile.AtEndOfStream){
				...
				if((oKey = io_ini_iskey(sLine,oFile.Line)) && oKey.name == sKeyName){
					oTmpFile.WriteLine(sKeyName + "=" + sKeyValue); // If exist, change key value
					break;
				}
					...
					if(oFile.AtEndOfStream){
						...
						oTmpFile.WriteLine(sKeyName + "=" + sKeyValue); // Add key value at end
...
function io_ini_getkeylines(sFile){
	try{
		if(oFso.FileExists(sFile)){
			...
			while(!oFile.AtEndOfStream){
				var sLine = oFile.ReadLine()
				if(sLine && js_str_isdefined(sLine) && sLine.substring(0,1) != ";" && sLine.substring(0,1) != "#"){
					if(oLine = io_ini_iskey(sLine,oFile.Line)){
						a.push(oLine);
					}
				}
